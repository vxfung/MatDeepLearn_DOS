...
def train(model, optimizer, loader, loss_method, loss_features, rank):
    model.train()
    loss_all = 0
    loss_dos_l1_all = 0
    count = 0
    for data in loader:
        ...
        dos_loss = getattr(F, loss_method)(dos_out, data.dos_scaled)
        scaling_loss = getattr(F, loss_method)(scaling, data.scaling_factor)

        output_cumsum = torch.cumsum(dos_out, axis=1)
        dos_cumsum = torch.cumsum(data.dos_scaled, axis=1)
        dos_cumsum_loss = getattr(F, loss_method)(output_cumsum, dos_cumsum) 
        
        scaled_dos = dos_out * scaling.view(-1,1).expand_as(dos_out)
        ...
        loss_all += loss_sum.detach() * dos_out.size(0)
        loss_sum.backward()

        loss_dos_l1 = getattr(F, "l1_loss")(scaled_dos, data.y)                
        loss_dos_l1_all += loss_dos_l1.detach() * dos_out.size(0)
        
        optimizer.step()
        count = count + dos_out.size(0)

    loss_all = loss_all / count
    loss_dos_l1_all = loss_dos_l1_all / count
    return loss_all, loss_dos_l1_all
...
def evaluate(loader, model, loss_method, loss_features, rank, out=False, subset=None):
    ...
    for data in loader:
        ...
        with torch.no_grad():
            ...
            features_loss = getattr(F, loss_method)(data.dos_features, features.to(data.dos_features.device), reduction="none")                           
            if out == True:                                                            
                if count == 0:
                    ids = [item for sublist in data.structure_id for item in sublist]
                    ids = [item for sublist in ids for item in sublist]
                    predict = scaled_dos.data.cpu().numpy()
                    target = data.y.cpu().numpy()
                    elements = data.z.cpu().numpy()
                    loss_l1 = np.mean(getattr(F, "l1_loss")(scaled_dos, data.y, reduction='none').cpu().numpy(), axis=1).flatten()
                    loss_rmse = np.sqrt(np.mean(getattr(F, "mse_loss")(scaled_dos, data.y, reduction='none').cpu().numpy(), axis=1).flatten())
                    
                    features_target = data.dos_features.cpu().numpy()
                    features_predict = features.cpu().numpy()
                    features_loss_all = features_loss.cpu().numpy()
                    ...
                    predict = np.concatenate(
                        (predict, scaled_dos.data.cpu().numpy()), axis=0
                    )
                    target = np.concatenate((target, data.y.cpu().numpy()), axis=0)
                    elements = np.concatenate((elements, data.z.cpu().numpy()), axis=0)
                    loss_l1= np.concatenate((loss_l1, np.mean(getattr(F, "l1_loss")(scaled_dos, data.y, reduction='none').cpu().numpy(), axis=1).flatten()), axis=0)
                    loss_rmse= np.concatenate((loss_rmse, np.sqrt(np.mean(getattr(F, "mse_loss")(scaled_dos, data.y, reduction='none').cpu().numpy(), axis=1).flatten())), axis=0)
                    
                    features_target = np.concatenate((features_target, data.dos_features.cpu().numpy()), axis=0)
                    features_predict = np.concatenate((features_predict, features.cpu().numpy()), axis=0)
                    features_loss_all = np.concatenate((features_loss_all, features_loss.cpu().numpy()), axis=0)
    ...
    loss_dos_l1_all = loss_dos_l1_all / count
    if out == True:
        features_out =  np.column_stack((ids, elements, features_loss_all, features_target, features_predict))
        if subset == None:
            print(len(ids), target.shape, predict.shape)
            test_out = np.column_stack((ids, elements, loss_l1, loss_rmse, target, predict))
            #test_out = np.column_stack((ids, elements, loss_l1, loss_rmse))
        else: 
            print(len(ids), target.shape, predict.shape)
            test_out = np.column_stack((ids[0:subset], elements[0:subset], loss_l1[0:subset], loss_rmse[0:subset], target[0:subset,:], predict[0:subset,:]))
        return loss_all,  loss_dos_l1_all, test_out, features_out        
    elif out == False:
        return loss_all,  loss_dos_l1_all
...
def trainer(
    ...
    train_error = train_error_dos_only = val_error = val_error_dos_only = test_error = epoch_time = float("NaN")
    ...
    for epoch in range(1, epochs + 1):
        ...
        ##Train model
        train_error, train_error_dos_only = train(model, optimizer, train_loader, loss, features_loss, rank=rank)
        if rank not in ("cpu", "cuda"):
        ...
        if val_loader != None and rank in (0, "cpu", "cuda"):
            if rank not in ("cpu", "cuda"):
                val_error, val_error_dos_only = evaluate(
                    val_loader, model.module, loss, features_loss, rank=rank, out=False
                )
            else:
                val_error, val_error_dos_only = evaluate(val_loader, model, loss, features_loss, rank=rank, out=False)
        ...
        if epoch % verbosity == 0:
            if rank in (0, "cpu", "cuda"):
                print(
                    "Epoch: {:04d}, Learning Rate: {:.6f}, Training Combined Error: {:.5f}, Val Combined Error: {:.5f}, Training MAE Error: {:.5f}, Val MAE Error: {:.5f}, Time per epoch (s): {:.5f}".format(
                        epoch, lr, train_error, val_error, train_error_dos_only, val_error_dos_only, epoch_time
                    )
                )
...
##Write results to csv file
def write_results_features(output, filename):
    shape = output.shape
    with open(filename, "w") as f:
        csvwriter = csv.writer(f)
        for i in range(0, len(output)):
...
def train_regular(
    ...
    if rank in (0, "cpu", "cuda"):
        ...
        train_error = val_error = test_error = train_error_dos_only = val_error_dos_only = test_error_dos_only = float("NaN")
        ...
        train_error, train_error_dos_only = evaluate(
            train_loader, model, training_parameters["loss"], training_parameters["features_loss"], rank, out=False
        )
        print("Train Error: {:.5f}".format(train_error_dos_only))

        #Get val error
        if val_loader != None:
            val_error, val_error_dos_only = evaluate(
                val_loader, model, training_parameters["loss"], training_parameters["features_loss"], rank, out=False
            )
            print("Val Error: {:.5f}".format(val_error_dos_only))

        ##Get test error
        if test_loader != None:
            test_error, test_error_dos_only, test_out, features_out = evaluate(
                test_loader, model, training_parameters["loss"], training_parameters["features_loss"], rank, out=True, subset=8000
                #test_loader, model, training_parameters["loss"], training_parameters["features_loss"], rank, out=True
            )
            print("Test Error: {:.5f}".format(test_error_dos_only))
        ...
        if job_parameters["write_output"] == "True":
            ...
            if test_loader != None:
                ...
                write_results_features(
                    features_out, str(job_parameters["job_name"]) + "_test_outputs_features.csv"
                )                
        ...
        error_values = np.array((torch.tensor(train_error).cpu(), torch.tensor(val_error).cpu(), torch.tensor(test_error).cpu()))
...
def predict(dataset, loss, job_parameters=None):
    ...
    subset=100    
    if subset == None:
        print(len(ids), predict.shape)
        test_out = np.column_stack((ids, elements, predict))
    else: 
        print(len(ids), predict.shape)
        test_out = np.column_stack((ids[0:subset], elements[0:subset], predict[0:subset,:]))      
    ...
    if job_parameters["write_output"] == "True":
        ...
        with open(str(job_parameters["job_name"]) + "_predicted_outputs.csv", "w") as f:
            csvwriter = csv.writer(f)
...
def tune_trainable(config, checkpoint_dir=None, data_path=None):
    ...
    for epoch in range(1, model_parameters["epochs"] + 1):
        ...
        train_error, train_out = train(
            model, optimizer, train_loader, training_parameters["loss"], training_parameters["features_loss"], rank=rank
        )

        val_error, val_out = evaluate(
            val_loader, model, training_parameters["loss"], training_parameters["features_loss"], rank=rank, out=False
        )
